...
ulong zRandInitSeedMT(zRandMT *mt, ulong seed)
{
  uint i;
  ulong s;

  if( !mt ) mt = __z_rand_mt = &__z_rand_mt_default;
  s = seed;
  mt->x[0] = ( s &= Z_RAND_MT_MAX );
  for( i=1; i<=Z_RAND_MT_HISTORY; i++ )
    mt->x[i] = s = ( Z_RAND_MT_MGC1 * (s^(s>>30)) + i ) & Z_RAND_MT_MAX;
  mt->index = 0;
...
ulong zRandInitMT(zRandMT *mt)
{
  static ulong seed;

  seed = time( NULL );
  if( mt ) seed++;
  return zRandInitSeedMT( mt, seed );
}
